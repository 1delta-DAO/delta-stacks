...
const VAULT    = `${deployer}.vault-stx-v5-4`
...
describe('vault-v5-4 — auto-allocation: proportional deposit split', () => {
...
describe('vault-v5-4 — overflow and underflow protection', () => {
...
describe('vault-v5-4 — safe rounding', () => {
...
describe('vault-v5-4 — proportional withdrawal (V1 share -> idle)', () => {

  it('withdraw pulls proportionally from idle + zest only, granite stays unchanged', () => {
    registerBoth()
    mint(deployer, AMOUNT)
    deposit(AMOUNT, deployer)
    ...
    // v5-4: split between idle + V2 only (available = 500K + 200K = 700K)
    // pull-v2 = floor(100_000 * 200_000 / 700_000) = 28_571
    // pull-idle = 100_000 - 28_571 = 71_429
    expect(vaultRead('get-alloc-granite')).toBe(300_000n)  // UNCHANGED
    expect(vaultRead('get-alloc-zest-v2')).toBe(200_000n - 28_571n)
    expect(vaultRead('get-total-assets')).toBe(BigInt(AMOUNT - withdrawAmt))
  })

  it('withdraw works when 0 idle but V2 has capital (pulls from V2 only)', () => {
    registerBoth()
    mint(deployer, AMOUNT)
    deposit(AMOUNT, deployer)
    ...
    // v5-4: available = idle(0) + V2(400K) = 400K
    // Withdraw 200K: pull-v2 = floor(200K * 400K / 400K) = 200K, all from V2
    const result = withdraw(200_000, wallet2, deployer)
    expect(result.result.type).toBe(ClarityType.ResponseOk)
    expect(tokenBalance(wallet2)).toBe(200_000n)
    expect(vaultRead('get-alloc-granite')).toBe(600_000n)  // UNCHANGED
    expect(vaultRead('get-alloc-zest-v2')).toBe(200_000n)
  })

  it('withdraw fails when amount exceeds idle + V2 (V1 capital locked)', () => {
    registerBoth()
    mint(deployer, AMOUNT)
    deposit(AMOUNT, deployer)

    // Deploy 900K to V1, 0 to V2, 100K idle
    deployToGranite(900_000)

    // Try to withdraw 200K but only 100K available (idle only, no V2)
    const result = withdraw(200_000, wallet2, deployer)
    expect(result.result.type).toBe(ClarityType.ResponseErr)
  })
  ...
  it('full redeem works when enough is in idle + V2', () => {
    registerBoth()
    mint(deployer, AMOUNT)
    deposit(AMOUNT, deployer)

    // Only deploy to V2 (no V1) so full redeem works
    deployToZest(500_000)
  ...
  it('both users redeem fully when capital is in idle + V2', () => {
    registerBoth()
    mint(deployer, AMOUNT)
    deposit(AMOUNT, deployer)

    // Deploy only to V2
    deployToZest(800_000)
  ...
  it('withdraw with V1 deployed: only V2 decreases, granite unchanged', () => {
    registerBoth()
    mint(deployer, AMOUNT)
    deposit(AMOUNT, deployer)

    // Deploy to both, but keep enough idle for V1's share
    deployToGranite(200_000)
    deployToZest(400_000)
    // idle = 400_000, granite = 200_000, zest = 400_000
    ...
    expect(totalAfter).toBe(totalBefore - 100_000n)

    // Granite unchanged, only zest decreased
    expect(vaultRead('get-alloc-granite')).toBe(200_000n)
    expect(vaultRead('get-alloc-zest-v2')).toBeLessThan(400_000n)
  })

  it('v5-4: complete-v1-recall updates granite bookkeeping', () => {
    registerBoth()
    mint(deployer, AMOUNT)
    deposit(AMOUNT, deployer)
    deployToGranite(500_000)
    ...
    // Simulate external manager recall: tokens already in idle
    simnet.callPublicFn(VAULT, 'complete-v1-recall', [Cl.uint(200_000)], deployer)

    expect(vaultRead('get-alloc-granite')).toBe(300_000n)
    // total-assets unchanged (STX moved from granite bookkeeping to idle)
    expect(vaultRead('get-total-assets')).toBe(BigInt(AMOUNT))
  })

  it('v5-4: complete-v1-deploy updates granite bookkeeping', () => {
    registerBoth()
    mint(deployer, AMOUNT)
    deposit(AMOUNT, deployer)

    simnet.callPublicFn(VAULT, 'complete-v1-deploy', [Cl.uint(300_000)], deployer)
...
describe('vault-v5-4 — yield sync + auto-allocation interaction', () => {
...
describe('vault-v5-4 — deposit succeeds even without valid adapter trait', () => {
...
describe('vault-v5-4 — share price correctness', () => {
...
describe('vault-v5-4 — access control', () => {
...
describe('vault-v5-4 — recall (market -> idle)', () => {
...
describe('vault-v5-4 — performance fees', () => {
  ...
  it('fee recipient can redeem shares for tokens', () => {
    ...
    mint(deployer, 5_000_000)
    deposit(5_000_000, deployer)
    // v5-4: deploy to V2 only so redeem can pull from idle + V2
    deployToZest(4_000_000)

    // Yield on zest: 200K -> 20% fee = 40K
    mint(ADAPTER2, 200_000)
  ...
  it('v5: withdraw does NOT auto-sync (sync removed for stack depth safety)', () => {
    registerBoth()
    simnet.callPublicFn(VAULT, 'set-fee-bps', [Cl.uint(1000)], deployer)
    simnet.callPublicFn(VAULT, 'set-fee-recipient', [Cl.principal(wallet2)], deployer)
    ...
    // v5: Withdraw does NOT trigger auto-sync (removed to save stack frames)
    withdraw(500_000, deployer, deployer)

    // Fee shares are NOT minted during withdraw -- yield captured on next deposit or explicit sync
    expect(vaultShares(wallet2)).toBe(0n)
  })
...
describe('vault-v5-4 — V2-only rebalancing + V1 bookkeeping', () => {

  it('reallocate moves V2 capital (recall + deploy)', () => {
    registerBoth()
    mint(deployer, AMOUNT)
    deposit(AMOUNT, deployer)
    deployToZest(500_000)

    // Recall 200K from zest to idle
    const result = simnet.callPublicFn(
      VAULT, 'reallocate',
      [Cl.uint(0), Cl.uint(200_000), Cl.uint(0), Cl.uint(0), graniteArg, zestArg],
      deployer,
    )
    expect(result.result.type).toBe(ClarityType.ResponseOk)
    expect(vaultRead('get-alloc-zest-v2')).toBe(300_000n)
  })
  ...
  it('reallocate V2 recall then deploy', () => {
    registerBoth()
    mint(deployer, AMOUNT)
    deposit(AMOUNT, deployer)
    deployToZest(800_000)

    // Recall 300K from V2, deploy 300K back
    const result = simnet.callPublicFn(
      VAULT, 'reallocate',
      [Cl.uint(0), Cl.uint(300_000), Cl.uint(0), Cl.uint(300_000), graniteArg, zestArg],
      deployer,
    )
    expect(result.result.type).toBe(ClarityType.ResponseOk)
    expect(vaultRead('get-alloc-zest-v2')).toBe(800_000n) // net zero change
  })
  ...
  it('reallocate preserves total-assets bookkeeping', () => {
    ...
    simnet.callPublicFn(
      VAULT, 'reallocate',
      [Cl.uint(0), Cl.uint(200_000), Cl.uint(0), Cl.uint(200_000), graniteArg, zestArg],
      deployer,
    )
  ...
  it('delegated allocator can reallocate V2', () => {
    registerBoth()
    mint(deployer, AMOUNT)
    deposit(AMOUNT, deployer)
    deployToZest(500_000)
  ...
  it('reallocate does not affect share price', () => {
    ...
    // Reallocate V2: recall 500K then deploy 500K (no-op)
    simnet.callPublicFn(
      VAULT, 'reallocate',
      [Cl.uint(0), Cl.uint(500_000), Cl.uint(0), Cl.uint(500_000), graniteArg, zestArg],
      deployer,
    )
...
describe('vault-v5-4 — initialize', () => {
