...
- Use Chrome DevTools MCP (`npx -y chrome-devtools-mcp@latest`) for browser console logs, wallet interactions, network requests, and runtime errors.
...
### Run Tests
Execute and debug Clarity contract tests:
1. Verify Clarinet project setup and test configuration exist
2. Run `clarinet check` to compile contracts first
3. Run tests via the test command in package.json (usually `npm test` or `npx vitest`)
4. Diagnose common issues: missing setup files, type mismatches (`Cl.uint()`, `Cl.principal()`), block height problems, undefined `simnet`
5. Generate and interpret coverage reports

### Deploy Contract
Guided deployment to testnet or mainnet:
1. Pre-flight: `clarinet check`, run tests, review coverage
2. Choose network (testnet recommended first)
3. Testnet: generate deployment plan, get faucet STX, apply deployment
4. Mainnet: requires prior testnet verification, security checklist, explicit confirmation
5. Contracts are immutable once deployed — bugs are permanent

### Start Dev Server
Launch local development environment:
1. Verify Docker is running and `Clarinet.toml` exists
2. Start devnet: `clarinet devnet start --no-dashboard`
3. Health check: query `http://localhost:20443/v2/info`
4. Detect and start frontend dev server from package.json
5. Set up Chrome DevTools MCP for browser debugging
6. Devnet requires Docker with 4GB+ RAM
