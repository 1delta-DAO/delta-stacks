;; vault-stx-v4.clar  (simnet testable version)
;;
;; Simnet-testable build of the v4 vault fix.  Uses mock-token and mock-adapter
;; so the calc-helper refactoring can be validated against the full v3 test suite.
;;
;; v4 change: withdraw/redeem proportional math is extracted into private helpers
;; (calc-withdraw-by-amount, calc-withdraw-by-shares) so the deeply nested let
;; frames unwind BEFORE adapter contract-calls, avoiding MaxStackDepthReached
;; when the adapter internally calls Zest V1 borrow-helper.withdraw.
...
(define-data-var virtual-offset uint u1000000)  ;; 10^decimals, default 10^6
(define-data-var vault-initialized bool false)

;; --- Configurable SIP-010 share metadata ---
(define-data-var vault-name     (string-ascii 32) "1delta Vault v4")
...
(define-read-only (get-adapter-granite)
  (var-get adapter-granite-usdcx))

(define-read-only (get-adapter-zest-v2)
...
;; ---------------------------------------------------------------------------
;; v4: Proportional withdrawal math helpers
;; ---------------------------------------------------------------------------
;; These isolate the deeply-nested let-chain that computes shares-to-burn
;; and per-market pull amounts.  Once the helper returns, its stack frames
;; are released, so the subsequent adapter contract-calls execute at a
;; shallow depth -- avoiding MaxStackDepthReached.

;; Compute withdraw params from an asset amount (for withdraw).
(define-private (calc-withdraw-by-amount (amount uint) (owner principal))
  (let ((supply (ft-get-supply vault-shares))
        (total  (var-get total-assets-bookkeeping))
          ...
          (let ((pull-idle   (/ (* amount idle-book) total))
            ...
            (let ((remaining (- amount pull-idle)))
              (let ((pull-granite (if (> ag-plus-az u0)
                                     ...
                                     u0)))
                (ok { shares-to-burn: shares-to-burn,
                      pull-granite: pull-granite,
                      pull-zest-v2: (- remaining pull-granite) })))))))))

;; Compute withdraw params from a share count (for redeem).
(define-private (calc-withdraw-by-shares (shares uint) (owner principal))
  (let ((supply (ft-get-supply vault-shares))
        (total  (var-get total-assets-bookkeeping))
        (bal    (ft-get-balance vault-shares owner))
        (ag     (let ((raw (var-get alloc-granite)))
...
;; Deposit `amount` on behalf of `owner`, mint shares, auto-allocate.
    ...
    (let ((sg (if (> (var-get alloc-granite) dust-threshold)
      ...
      (let ((supply (ft-get-supply vault-shares))
        ...
        (let ((shares (/ (* amount (+ supply voff)) (+ total voff))))
          ...
          (let ((a1 (let ((raw (var-get alloc-granite)))
                      (if (<= raw dust-threshold) u0 raw)))
...
;; ---------------------------------------------------------------------------
;; Withdraw / Redeem -- v4 flattened (stack-safe)
;; ---------------------------------------------------------------------------
    ...
    (let ((sg (if (> (var-get alloc-granite) dust-threshold)
          ...
          (sz (if (> (var-get alloc-zest-v2) dust-threshold)
               (begin
                 ...
                 (try! (do-sync-zest-v2 zest-v2)))
               u0)))
    ;; 2. Compute proportional amounts (deep nesting inside helper, unwinds).
    (let ((params (try! (calc-withdraw-by-amount amount owner))))
      (let ((shares-to-burn (get shares-to-burn params))
            (pull-granite   (get pull-granite params))
            (pull-zest-v2   (get pull-zest-v2 params)))
        ;; 3. Validate adapters for non-zero pulls.
        (if (> pull-granite u0)
          (asserts! (is-eq (some (contract-of granite))
                           (var-get adapter-granite-usdcx))
                    err-invalid-adapter)
        ...
        (if (> pull-zest-v2 u0)
          (asserts! (is-eq (some (contract-of zest-v2))
                    ...
                    err-invalid-adapter)
          true)
        ;; 4. Execute adapter withdrawals (shallow stack).
        (let ((rg (if (> pull-granite u0)
              ...
              (rz (if (> pull-zest-v2 u0)
                     ...
                     u0)))
          ;; 5. Final bookkeeping and transfer.
    ...
    (let ((sg (if (> (var-get alloc-granite) dust-threshold)
          ...
          (sz (if (> (var-get alloc-zest-v2) dust-threshold)
               (begin
                 ...
                 (try! (do-sync-zest-v2 zest-v2)))
               u0)))
    (let ((params (try! (calc-withdraw-by-shares shares owner))))
      (let ((amt          (get amt params))
            (pull-granite (get pull-granite params))
            (pull-zest-v2 (get pull-zest-v2 params)))
        (if (> pull-granite u0)
          (asserts! (is-eq (some (contract-of granite))
