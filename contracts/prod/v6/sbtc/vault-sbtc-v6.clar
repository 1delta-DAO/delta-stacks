...
;; v6 changes
;; ----------
;; Full proportional withdrawal from idle + V1 + V2 (not the v5 idle+V2 only
;; approach).  V1 collateral must be disabled via adapter.disable-collateral()
;; after each deploy so borrow-helper.withdraw skips the deep health-factor
;; check, keeping the stack under 64 frames.
;;
;; - User withdraw/redeem pulls proportionally from idle + V1 + V2.
;; - Data vars for scratch: wd-shares-to-burn, wd-pull-v1, wd-pull-v2, wd-amt.
;; - No sync on withdraw path.
;; - deploy-to-zest-v1 stays in vault.
;; - complete-v1-recall / complete-v1-deploy for external manager bookkeeping.
;; - sync-v1-yield public function.
;; - reallocate-v2 (V2 only).
;;
...
(define-data-var vault-name     (string-ascii 32) "1delta sBTC Vault v6")
...
;; ---------------------------------------------------------------------------
;; v6: Scratch data vars for withdrawal params
;; ---------------------------------------------------------------------------
...
;; ---------------------------------------------------------------------------
;; Withdraw / Redeem -- v6 data-var approach (minimal stack depth)
;; ---------------------------------------------------------------------------
...
;; Withdraw exactly `amount` assets via SIP-010 path.
...
;; Redeem exactly `shares` via SIP-010 path.
...
;; Note: do-deallocate is NOT used for withdraw/redeem in v6 (inlined for
