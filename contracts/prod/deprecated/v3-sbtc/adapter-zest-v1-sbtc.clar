;; adapter-zest-v1-sbtc.clar
;;
;; Adapter bridging the sBTC vault to the Zest V1 sBTC lending pool (Aave-like).
;; Implements lending-adapter-trait so the vault can deposit, withdraw, and
;; query position value through a uniform interface.
;;
;; Unlike the wSTX adapter, sBTC is a standard SIP-010 token with no
;; wrapping/unwrapping. The adapter simply transfers sBTC between vault and
;; Zest V1 via the borrow-helper.
;;
;; Pyth note
;; ---------
...
;; infinite.  We pass `none` for price-feed-bytes.
...
;; z-token for sBTC (receipt token, balance = supplied + accrued interest)
...
;; Oracle for BTC-based assets
(define-constant btc-oracle 'SP2VCQJGH7PHP2DJK7Z0V48AGBHQAW3R3ZW1QF4N.stx-btc-oracle-v1-4)
  ...
  (let ((vault tx-sender))
    ...
    (try! (as-contract
            (contract-call? 'SP2VCQJGH7PHP2DJK7Z0V48AGBHQAW3R3ZW1QF4N.borrow-helper-v2-1-7
              supply
              'SP2VCQJGH7PHP2DJK7Z0V48AGBHQAW3R3ZW1QF4N.zsbtc-v2-0         ;; lp (z-token)
...
;; Withdraw `amount` sBTC from Zest V1, sending proceeds to `recipient`.
;;
...
(define-public (adapter-withdraw (amount uint) (recipient principal))
  (begin
    ...
    ;; Zest V1 withdraw sends sBTC to owner (adapter), forward to recipient (vault)
    (try! (as-contract
            (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token
              transfer amount tx-sender recipient none)))
    (ok amount)))

;; Read the live sBTC value of this adapter's Zest V1 position.
;;
