...
;;
;; Allocator (vault-allocator, delegated by owner) -- may only move funds
...
;; Address permitted to register adapters, set allocator, and transfer ownership.
(define-data-var vault-owner principal tx-sender)

;; Address permitted to execute allocation operations (deploy / rebalance).
;; Defaults to deployer; owner can delegate to a bot or multisig.
...
;; Delegate allocation authority to a different principal (bot, multisig, etc.).
;; Only vault-owner may call.  The allocator can deploy and rebalance but
;; cannot register adapters or change ownership.
...
;; ---------------------------------------------------------------------------
;;
;; All four functions require tx-sender == vault-allocator.
