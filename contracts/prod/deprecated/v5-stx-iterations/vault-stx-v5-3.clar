...
(define-data-var vault-name     (string-ascii 32) "1delta STX Vault v5-3")
        ...
        (let ((idle-book (if (>= total (+ a1 a2)) (- total (+ a1 a2)) u0)))
          (let ((pull-idle   (/ (* amount idle-book) total))
            ...
            (let ((remaining (- amount pull-idle)))
              ;; V1's share is computed but redirected to V2
              (let ((v1-share (if (> a1-plus-a2 u0)
                                  (/ (* remaining a1) a1-plus-a2)
                                  u0)))
                (var-set wd-shares-to-burn shares-to-burn)
                (var-set wd-pull-v1 u0)  ;; never pull from V1
                (var-set wd-pull-v2 remaining)  ;; V2 covers V1's share
                (ok true)))))))))
