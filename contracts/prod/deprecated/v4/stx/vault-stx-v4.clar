...
;; v4 changes (from v3)
;; --------------------
;; - Fixed MaxStackDepthReached on withdraw/redeem by extracting the
;;   proportional-math into private helpers (calc-withdraw-by-amount,
;;   calc-withdraw-by-shares).  The helpers' nested lets unwind BEFORE
;;   the adapter contract-calls, keeping the call stack shallow when
;;   borrow-helper.withdraw runs internally.
;;
...
(define-data-var vault-name     (string-ascii 32) "1delta STX Vault v4")
...
;; shallow depth -- avoiding the MaxStackDepthReached error that occurred
;; in v3 when the Zest V1 borrow-helper.withdraw ran inside 12+ nested lets.

;; Compute withdraw params from an asset amount (for withdraw / withdraw-stx).
;; Returns { shares-to-burn, pull-v1, pull-v2 }.
...
;; Compute withdraw params from a share count (for redeem / redeem-for-stx).
;; Returns { amt, pull-v1, pull-v2 }.
...
;; ---------------------------------------------------------------------------
;; The proportional math is computed in calc-withdraw-by-amount /
;; calc-withdraw-by-shares.  Once those helpers return, the stack unwinds,
;; so the adapter calls below execute at depth ~3 instead of ~12.
...
(define-public (withdraw (amount uint) (receiver principal) (owner principal)
  ...
  (begin
    ...
    (asserts! (is-eq (contract-of token) (var-get base-asset)) err-invalid-asset)
    ;; 1. Sync yields (sequential calls, each unwinds before the next).
    (let ((s1 (if (> (var-get alloc-zest-v1) dust-threshold)
               (begin
                 (asserts! (is-eq (some (contract-of zest-v1))
                                  (var-get adapter-zest-v1-wstx))
    ...
    (let ((params (try! (calc-withdraw-by-amount amount owner))))
      (let ((shares-to-burn (get shares-to-burn params))
            (pull-v1        (get pull-v1 params))
            (pull-v2        (get pull-v2 params)))
        ...
        (if (> pull-v2 u0)
          (asserts! (is-eq (some (contract-of zest-v2))
                    ...
                    err-invalid-adapter)
          true)
        ;; 4. Execute adapter withdrawals (shallow stack = no overflow).
