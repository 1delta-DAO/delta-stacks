;; vault-sbtc-v5.clar
;;
;; ERC-4626-style yield vault for sBTC -- SIP-010 base asset only.
;; Vault shares are a SIP-010 fungible token, transferable between users.
;;
;; Markets: Zest V1 (zsbtc, Aave-like pool) + Zest V2 (v0-vault-sbtc, ERC-4626 vault).
;; No Granite market for sBTC.
;;
;; sBTC note
;; ---------
;; sBTC is a native SIP-010 token on Stacks.  Unlike the STX vault there are
;; no native-STX convenience functions (deposit-stx, withdraw-stx, redeem-for-stx).
;; All deposits and withdrawals go through the SIP-010 token trait.
;;
;; v5 design (same as STX v5-5)
;; ----------------------------
...
(define-data-var virtual-offset uint u100000000)
(define-data-var vault-initialized bool false)

(define-data-var vault-name     (string-ascii 32) "1delta sBTC Vault v5")
...
(define-read-only (get-adapter-zest-v1-sbtc) (var-get adapter-zest-v1-sbtc))
(define-read-only (get-adapter-zest-v2-sbtc) (var-get adapter-zest-v2-sbtc))
...
;; ---------------------------------------------------------------------------
;; V1 is included in the proportional math (share price reflects V1 capital),
;; but V1's proportional share is redirected to V2 for the actual pull.
;; The vault never calls borrow-helper.withdraw -- only idle + V2 are tapped.
;;
;; If idle + V2 can't cover the full withdrawal amount, the tx fails with
;; err-insufficient-balance.  The allocator must recall V1 capital first.

;; For withdraw: amount -> data vars
  ...
  (let ((supply (ft-get-supply vault-shares))
    ...
    (let ((shares (/ (* amount (+ supply voff)) (+ total voff))))
      (let ((shares-to-burn
        ...
        ;; Split proportionally between idle and V2 only (V1 excluded from pull).
        ;; available = idle + V2.  If amount > available, fails later on balance check.
        (let ((idle-book (if (>= total (+ a1 a2)) (- total (+ a1 a2)) u0)))
          (let ((available (+ idle-book a2)))
            (let ((pull-v2 (if (> available u0) (/ (* amount a2) available) u0)))
              (var-set wd-shares-to-burn shares-to-burn)
              (var-set wd-pull-v1 u0)
              (var-set wd-pull-v2 pull-v2)
              (ok true))))))))

;; For redeem: shares -> data vars
(define-private (prepare-withdraw-by-shares (shares uint) (owner principal))
  (let ((supply (ft-get-supply vault-shares))
    ...
    (let ((amt (/ (* shares (+ total voff)) (+ supply voff))))
      ...
      (let ((idle-book (if (>= total (+ a1 a2)) (- total (+ a1 a2)) u0)))
        (let ((available (+ idle-book a2)))
          (let ((pull-v2 (if (> available u0) (/ (* amt a2) available) u0)))
            (var-set wd-amt amt)
            (var-set wd-pull-v1 u0)
            (var-set wd-pull-v2 pull-v2)
            (ok true)))))))
...
;; Deposit `amount` sBTC on behalf of `owner`, mint shares, auto-allocate.
...
;; Withdraw exactly `amount` assets via sBTC (SIP-010 path).
;; V1 included in math but only idle + V2 are tapped.
(define-public (withdraw (amount uint) (receiver principal) (owner principal)
  ...
  (begin
    ...
    ;; V2 pull (covers V1's share too -- no borrow-helper call)
...
;; Redeem exactly `shares` via sBTC (SIP-010 path).
...
;; Called after zest-v1-manager.recall() moves sBTC from V1 adapter to vault.
;; Updates alloc-zest-v1 bookkeeping.  The sBTC is already in the vault's idle.
...
;; Called after zest-v1-manager.deploy() moves sBTC from vault idle to V1 adapter.
;; Updates alloc-zest-v1 bookkeeping.  The sBTC has already left the vault.
...
;; --- V2 rebalancing (no V1 involvement) ---
